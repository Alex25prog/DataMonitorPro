#ifndef BYBITTRIANGULARARBITRAGEMONITOR_H
#define BYBITTRIANGULARARBITRAGEMONITOR_H
...
/*
 * Мониторинг треугольного арбитража на Bybit spot.
 * Цикл: USDT -> BTC -> ETH -> USDT (прямой) и USDT -> ETH -> BTC -> USDT (обратный).
 *
 * Зеркало TriangularArbitrageMonitor (Binance) — та же схема расчёта,
 * тот же публичный интерфейс для QML, но источник данных другой: Bybit не
 * присылает единый "bookTicker" — вместо него используется поток лучшего
 * bid/ask первого уровня стакана (orderbook.1.{symbol}), который для спота
 * ВСЕГДА приходит полным снепшотом (не дельтой), так что не нужно
 * накапливать локальный стакан — просто берём первый элемент массивов b/a
 * из каждого сообщения.
 *
 * Только расчёт и отображение — реальные ордера не выставляются.
 */
class BybitTriangularArbitrageMonitor : public QObject
{
    Q_OBJECT

    Q_PROPERTY(bool isRunning READ isRunning NOTIFY runningChanged)

    Q_PROPERTY(QString symbolA READ symbolA CONSTANT) // BTCUSDT
...
public:
    explicit BybitTriangularArbitrageMonitor(QObject *parent = nullptr);
    ~BybitTriangularArbitrageMonitor();
...
private:
    ...
    // Тот же классический треугольник, что и у Binance-версии.
    ...
    // % комиссии за одну сделку (типичный Bybit spot taker fee — 0.1%).
...
#endif // BYBITTRIANGULARARBITRAGEMONITOR_H